...
const EngineeringTab = ({ engineering }) => {
      ...
      <div>
        ...
        <ul className="space-y-3">
          {engineering.whatItActuallyMeans.map((item, idx) => (
            <li key={idx} className="flex items-start text-sm text-slate-300 bg-slate-950 p-3 rounded-lg border border-slate-800">
              <ChevronRight size={16} className="mr-2 mt-0.5 text-blue-400 shrink-0" />
              {item}
            </li>
          ))}
        </ul>
